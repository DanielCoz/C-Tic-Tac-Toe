...
void display_board_content(game* g) {
	...
	for (int k = 0; k < 3; k++) {
		...
		printf("        |---|---|---|        |---|---|---|\n");
		printf("        | %d | %d | %d |        | %c | %c | %c |\n", indicativeIndex, indicativeIndex + 1, indicativeIndex + 2, (*g).board[boardRow][0], (*g).board[boardRow][1], (*g).board[boardRow][2]);
...
int user_get_movement_index(game* g) {
	...
	boolean validMovementIndex = False;
	...
	while (validMovementIndex == False) {
		...
		if (res < 0 || res > 8) {
			...
			validMovementIndex  = is_position_empty(g,res);

			if (validMovementIndex == False) {
				printf("\n        The position is already busy. Try again!\n");
			}
...
int is_there_a_winner(game* g) {
	...
	if (((*g).board[0][0] == (*g).board[1][0]) && ((*g).board[1][0] == (*g).board[2][0])) {
		if ((*g).board[0][0] != ' ') {
			if ((*g).board[0][0] == 'X') {
				res = 1;
			}
	...
	if (((*g).board[0][1] == (*g).board[1][1]) && ((*g).board[1][1] == (*g).board[2][1])) {
		if ((*g).board[0][1] != ' ') {
	...
	if (((*g).board[0][2] == (*g).board[1][2]) && ((*g).board[1][2] == (*g).board[2][2])) {
		if ((*g).board[0][2] != ' ') {
			if ((*g).board[0][2] == 'X') {
				res = 1;
			}
...
void process_movement(game* g, int pos) {
	...
	if (winnerIndex == 0 && boardFull == True) {
		(*g).status = 5;
	}
	...
	if (winnerIndex == 0 && boardFull == False){
...
void play_game(char* p1, char* p2, int im) {
	...
	while ((*g).status == 1 || (*g).status == 2) {
		...
		int movIndex = get_next_movement_index(g);
		process_movement(g, movIndex);
	}
